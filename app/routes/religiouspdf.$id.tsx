...
export const loader: LoaderFunction = async (props: LoaderArgs) => {
  ...
  const data = await ApiCall({
    query: `
        query getReligiousById($id:Int!){
            getReligiousById(id:$id){
              id,
              name,
              address,
              mobile,
  ...
  const village = await ApiCall({
    query: `
      ...
      `,
    veriables: {
      id: parseInt(data.data.getReligiousById.village_id!),
    },
  });
  ...
  const ReligiousPdf = () => (
    <Document>
      <Page style={styles.body} size={"A4"}>
        ...
        <View style={styles.flexbox}>
          <View style={styles.flexbox1}>
            <Text style={styles.filenametext}>
              No. EST-RLG-
              {`0000${form.id}`.substring(`0000${form.id}`.length - 4)}
            </Text>
          </View>
        ...
        <View style={styles.dividertwo}></View>
        <View>
          <Text style={styles.heading}>Religious Event Permission</Text>
        </View>
        ...
        <View>
          <Text style={styles.para}>
            Subject: Permission for Religious Event
          </Text>
        </View>
        ...
        <View>
          <Text style={styles.para}>
            &nbsp;&nbsp;&nbsp;&nbsp; Please see the representation cited,
            wherein it has been requested to grant permission for a Peaceful
            Religious Event At {form.event_address} through the following route{" "}
            {form.route_info} From{" "}
            {new Date(form.from_date)
              .toJSON()
              .slice(0, 10)
              .split("-")
        ...
        {form.query_status == "APPROVED" ? (
          <>
            <View style={styles.dividerthree}></View>
            <View>
              <Text style={styles.centerpara}>
                {" "}
                Hence the request has been examined, considered and "Approved" :
                .
              </Text>
            </View>
          </>
          ...
          <>
            ...
            <View>
              <Text style={styles.centerpara}>
                Hence the request has been examined, Considered and Rejected
              </Text>
            </View>
          </>
        ) : null}
        ...
        <View>
          <Text style={styles.heading}>
            Please abide by the below mentioned conditions
          </Text>
        </View>
        ...
        <View>
          <Text style={styles.para}>
            {form.condition_to_follow.replace(/\\n/g, "\n")}
          </Text>
        </View>
...
export default ReligiousPdfView;

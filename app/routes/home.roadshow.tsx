...
const Roadshow: React.FC = (): JSX.Element => {
  ...
  const route_infoRef = useRef<HTMLInputElement>(null);
  ...
  const submit = async () => {
    const RoadshowScheme = z
      .object({
        ...
        event_address: z.string().nonempty("Event Address is required."),
        relation: z.string().nonempty("Applicant relation is required."),
    ...
    const roadshowScheme: RoadshowScheme = {
    ...
    if (parsed.success) {
      ...
      if (
        ...
        const data = await ApiCall({
          query: `
                    mutation createRoadshow($createRoadshowInput:CreateRoadshowInput!){
                        createRoadshow(createRoadshowInput:$createRoadshowInput){
                          id
                        }
                      }
                    `,
          veriables: {
            createRoadshowInput: {
              ...
              name: roadshowScheme.name,
              address: roadshowScheme.address,
              email: roadshowScheme.email,
              mobile: roadshowScheme.mobile,
              user_uid: roadshowScheme.user_uid,
              village_id: roadshowScheme.village_id,
              signature_url: sign_url.data,
              from_date: roadshowScheme.from_date,
              to_date: roadshowScheme.to_date,
              iagree: roadshowScheme.iagree,
    ...
    <>
      <div className="bg-white rounded-md shadow-lg p-4 my-4 w-full">
        ...
        <p className="text-center font-semibold text-lg md:text-xl text-gray-800">
          {" "}
          Subject : Request for Obtaining Roadshow Permission.{" "}
        </p>
        ...
        <div className="flex flex-wrap gap-4 gap-y-2 items-center px-4 py-2 my-2">
          ...
          <div className="flex-none flex gap-4 lg:flex-1 w-full lg:w-auto">
            <div className="hidden">
              <input
                type="file"
                ref={witness_1_url_Ref}
                onChange={(e) => handleLogoChange(e, setWitness_1_url)}
              />
            </div>
            <button
            ...
            >
              <div className="flex items-center gap-2">
                <Fa6SolidLink></Fa6SolidLink>{" "}
                {witness_1_url == null ? "Attach Doc." : "Update Doc."}
              </div>
            </button>
            {witness_1_url != null ? (
              <a
                target="_blank"
                href={URL.createObjectURL(witness_1_url)}
                className="py-1 w-full sm:w-auto flex items-center gap-2  text-white text-lg px-4 bg-green-500 text-center rounded-md font-medium"
                rel="noreferrer"
              >
                <Fa6SolidFileLines></Fa6SolidFileLines>
        ...
        <div className="flex flex-wrap gap-4 gap-y-2 items-center px-4 py-2 my-2">
          ...
          <div className="flex-none flex gap-4 lg:flex-1 w-full lg:w-auto">
            <div className="hidden">
              <input
                type="file"
                ref={witness_2_url_Ref}
                onChange={(e) => handleLogoChange(e, setWitness_2_url)}
              />
            </div>
            <button

...
//   Chart as ChartJS,
//   ArcElement,
//   Toolttype ip,on} frm "@remix-ru/node";
...
const DashboradCard: React.FC<DashboradCardProps> = (
  ...
  return (
    <div className="h-32 w-52 rounded-md bg-white shadow-lg border-2 flex flex-col">
      <p
        className={`grow text-2xl ${props.textcolor} font-semibold text-center`}
      >
        {props.title}
      </p>
      <p
        className={`grow text-5xl ${props.textcolor} font-semibold text-center`}
      >
        {props.value}
      </p>
      <Link
        to={props.link}
        className={`rounded-b-lg w-full py-2 ${props.color} text-white font-semibold text-xl inline-block text-center`}
      >
        VIEW
      </Link>
    </div>

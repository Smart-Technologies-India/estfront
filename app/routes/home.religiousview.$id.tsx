...
export const loader: LoaderFunction = async (props: LoaderArgs) => {
  ...
  return json({
    user: cookie,
    from_data: data.data.getReligiousById,
...
const ReligiousView: React.FC = (): JSX.Element => {
  ...
  const reject = async (id: number) => {
    ...
    if (!data.status) {
      ...
      await axios.post(
        `http://sms.smartechwebworks.com/submitsms.jsp?user=ESTDAMAN&key=7670b7cc9eXX&mobile=+91${from_data.mobile}&message=Permission%20for%20Application%20No.%20676767%20has%20been%20rejected.%20For%20more%20details%2C%20please%20visit%20our%20website.-Establishment%2CCollectorate%20Daman.&senderid=ESTDMN&accusage=1&entityid=1701175448046997959&tempid=1707175473123349384`
      );
    ...
    <>
      ...
      <div className="bg-white rounded-md shadow-lg p-4 my-4 w-full">
        ...
        <p className="text-center font-semibold text-xl text-gray-800">
          {" "}
          SUBJECT : Request for Obtaining Religious Information.{" "}
        </p>
        ...
        {notings.length == 0 ? (
          ...
          <>
            {user.role == "SHO" ? (
              <>
                {notings
                  .filter((val: any) => val.to_user.role == "SHO")
                    ...
                    return (
                      <div key={index}>
                        <QueryTabs
                          ...
                          doc={val.doc_url}
                          from_user={
                            val.from_user.role == "USER"
                              ? "User"
                              : val.from_user.name
                          }
                          to_user={
                            val.to_user.role == "USER"
                              ? "User"
                              : val.to_user.name
                          }
                        />

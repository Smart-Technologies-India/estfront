        ...
        for (let i = Math.max(1, props.activePage - maxButtonsToShow); i <= Math.min(props.getMaxPage(), props.activePage + maxButtonsToShow); i++) {
            buttons.push(
                <button
                    ...
                    className={`rounded-md border-2 border-gray-500 hover:border-blue-500 hover:text-blue-500 text-lg text-center w-8 h-8 grid place-items-center ${props.activePage === i ? 'bg-blue-500 bg-opacity-25 text-blue-500 border-blue-500' : ''
                        }`}
                >
                    {i}
                </button>
        ...
        <>
            <div className="flex items-center gap-2 w-full mt-4 flex-wrap justify-between gap-y-4">
                <div className="flex gap-2 items-center">
                    <p className="text-sm text-black font-normal text-left">Page {props.activePage}/{props.getMaxPage()}</p>
                    <div className="h-5 w-[1px] bg-gray-500"></div>
                    <p className="text-sm text-black font-normal text-left">Total {props.getTotalItemsLength()} item</p>
                </div>

                <div className="flex gap-1 items-center">
                    <button onClick={props.firstPage} className="rounded-md border-2 border-gray-500 hover:border-blue-500 hover:text-blue-500 text-lg text-center w-8 h-8 grid place-items-center">
                        <MaterialSymbolsKeyboardDoubleArrowLeft></MaterialSymbolsKeyboardDoubleArrowLeft>
                    </button>
                    <button onClick={props.prevPage} className="rounded-md border-2 border-gray-500 hover:border-blue-500 hover:text-blue-500 text-lg text-center w-8 h-8 grid place-items-center">
                        <CharmChevronLeft></CharmChevronLeft>
                    </button>
                    {renderPageButtons()}
                    <button onClick={props.nextPage} className="rounded-md border-2 border-gray-500 hover:border-blue-500 hover:text-blue-500 text-lg text-center w-8 h-8 grid place-items-center">
                        <CharmChevronRight></CharmChevronRight>
                    </button>
                    <button onClick={props.lastPage} className="rounded-md border-2 border-gray-500 hover:border-blue-500 hover:text-blue-500 text-lg text-center w-8 h-8 grid place-items-center">
                ...
                <div className="flex gap-2 items-center">
                    ...
                    <div className="relative">
                        ...
                        <div
                            className={`absolute bottom-0 -translate-y-10 bg-white  left-0 rounded-md shadow-sm w-32 overflow-y-hidden transition-all duration-500 ${isSelectPage ? "block" : "hidden"}`}>
                            {[5, 10, 25, 50, 100].map((val: number, index: number) => (
                                <p key={index} onClick={() => { handelPageChange(); props.ChangePerPage(val); }} className="rounded-md hover:bg-blue-500 hover:bg-opacity-25 cursor-pointer px-2 mx-2 my-1">{val} / Page</p>
                            ))}
                        </div>
                    </div>
                    <div className="h-5 w-[1px] bg-gray-500"></div>
